...
void loop()
{
  ...
  valueMinus1 = thisValue;
  thisValue = readADC_Avg(valueForStripe);

  pwm.setPWM(0, 0, valueMinus2);
  pwm.setPWM(1, 0, valueMinus1);
  pwm.setPWM(2, 0, thisValue);
  pwm.setPWM(3, 0, valueMinus1);
  pwm.setPWM(4, 0, valueMinus2);
  

  //Serial.println(readADC_Avg(valueForStripe));
  // Set the PWM for the LED stripes:
  for(int i = 0; i < 5; i++){
    //pwm.setPWM(i, map(readADC_Avg(valueForStripe),0,1024,0,4096), 0);
    pwm.setPWM(i, 0, readADC_Avg(valueForStripe));
    //pwm.setPWM(i, 0, (map(AN_Pot1_Raw,0,1024,0,4096) + (4096/16)*0) % 4096 );
  }
