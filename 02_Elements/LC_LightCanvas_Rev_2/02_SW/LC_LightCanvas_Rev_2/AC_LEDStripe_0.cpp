...
/**
 * @brief AC_LEDStripe_0 Constructor.
 * @param pinLed  Pin of the LED Stripe
 */
AC_LEDStripe_0::AC_LEDStripe_0(int iniR, int iniG, int iniB)
{
  // Pin Configuration
  _iniR = iniR;
  _iniG = iniG;
 ...
 */
void AC_LEDStripe_0::setFromSensor(int sensorRead){
 ...
 * @param colorCode Color to be set
 */
void AC_LEDStripe_0::setFromSensor_unitary(int sensorRead){
    //sensorRead=analogRead(A0);
    //sensorRead = map(analogRead(34], 0, 255, 0, 4096);
    sensorRead=sensorRead*2;
   // Serial.println(sensorRead);
   // delay(50);
    if((sensorRead>=450)&&(sensorRead<=550))
    {
      for (int i = 0; i <= _qLed; i++){
        leds[i]=CRGB (gamma8[0], gamma8[0], gamma8[255]);
       } 
    }
    else if((sensorRead>=400)&&(sensorRead<=450))
    {
      for (int i = 0; i <= _qLed; i++){
        leds[i]=CRGB (gamma8[153], gamma8[153], gamma8[0]);
       }
    }
    else if((sensorRead>=350)&&(sensorRead<=400))
     {
        for (int i = 0; i <= _qLed; i++){
        leds[i]=CRGB (gamma8[200], gamma8[50], gamma8[200]);
       }
     }
     else if((sensorRead>=300)&&(sensorRead<=350))
    {
      for (int i = 0; i <= _qLed; i++){
        leds[i]=CRGB (gamma8[10], gamma8[25], gamma8[217]);
       }
    }
     ...
     {
       for (int i = 0; i <= _qLed; i++){
        leds[i]=CRGB (gamma8[50], gamma8[50], gamma8[150]);
       }
     }
     else if((sensorRead>=250)&&(sensorRead<=275))
     {
       for (int i = 0; i <= _qLed; i++){
        leds[i]=CRGB (gamma8[230], gamma8[0], gamma8[10]);
       }
     }
    else if((sensorRead>=235)&&(sensorRead<=250))
     {
       for (int i = 0; i <= _qLed; i++){
        leds[i] =CRGB (gamma8[0], gamma8[160], gamma8[0]);
       }
     }
     else if((sensorRead>=200)&&(sensorRead<=235))
     {
       for (int i = 0; i <= _qLed; i++){
        leds[i] =CRGB (gamma8[1], gamma8[0], gamma8[1]);
       }
     }
    else
    {
       for (int i = 0; i <= _qLed; i++){
        leds[i] = CRGB ( gamma8[_iniR],gamma8[_iniG],gamma8[_iniB]);
       }
    }
   FastLED.show();
}
